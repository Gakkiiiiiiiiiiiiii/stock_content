...
def _insert_ignore(session, model, values: dict, conflict_columns: list) -> bool:
    ...
    if dialect == "postgresql":
        ...
        # psycopg does not promise a useful rowcount for INSERT .. ON CONFLICT
        # DO NOTHING.  A returned primary key is the authoritative indication
        # that this transaction won the immutable first-write race.
        primary_key = next(iter(model.__table__.primary_key.columns))
        result = session.execute(
            statement.on_conflict_do_nothing(index_elements=conflict_columns).returning(primary_key)
        )
