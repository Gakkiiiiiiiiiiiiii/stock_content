...
class PostgresFinancialEntityRepository:
    ...
    def replace(self, video_id: str, units: list[KnowledgeUnit]) -> None:
        with self._sessions.begin() as session:
            ...
            seen: set[str] = set()
            for unit in units:
                resolution = dict((unit.attributes or {}).get("entity_resolution") or {})
                for entity in (unit.attributes or {}).get("entities") or []:
                    raw = str(entity.get("entity_name") or entity.get("name") or entity.get("ticker") or "").strip()
                    if not raw:
                        continue
                    ticker = str(entity.get("ticker") or entity.get("entity_key") or "").strip()
                    key = self._canonical_key(ticker or raw)
                    identity = f"{raw}:{key}"
                    if identity in seen:
                        continue
                    seen.add(identity)
                    ...
                    session.add(
                        FinancialEntityRow(
                            ...
                            confidence=entity.get("confidence_score") or resolution.get("confidence"),
                            resolution_source=str(
                                entity.get("resolution_source") or resolution.get("resolution_source") or "pipeline"
                            ),
                        )
                    )
