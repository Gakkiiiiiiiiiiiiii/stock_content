...
class CrossVideoCorroboration:
    ...
    def annotate(self, units: list[dict], all_units: list[dict]) -> list[dict]:
        ...
        for item in all_units:
            ...
            if key != ("", "") and item.get("support_status") in {"SOURCE_SUPPORTED", "CROSS_MODAL_SUPPORTED", "EXTERNALLY_VERIFIED", "VALIDATED"}:
                groups[key].append(item)
        results = []
        for unit in units:
            key = (str(unit.get("subject_key") or ""), str(unit.get("predicate_key") or ""))
            corroborators = groups.get(key, [])
            sources = sorted({item.get("source_video_id") for item in corroborators if item.get("source_video_id") is not None})
            ...
            results.append(dict(unit) | {"attributes": (unit.get("attributes") or {}) | {"cross_video_corroboration": narrative}})
        return results
